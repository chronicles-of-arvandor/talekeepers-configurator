...
import { getFeatById, getFeatByName, getFeats, getFeatsDirectory, Feat } from '../../models/feats';
...
import { getEffects, getEffectsDirectory, FeatEffect } from '../../models/effects';

export function displayNewFeatChoiceMenu(name: string, text: string, amount: number, prerequisites: Prerequisite[], feats: Feat[], rl: readline.Interface) {
  menu(
    'New feat choice\n' + gray(`${feats.length} feats selected`),
    option('Name ' + gray(`(${name})`), () => {
      rl.question('Name: ', (newName) => {
        displayNewFeatChoiceMenu(newName, text, amount, prerequisites, feats, rl);
      });
    }),
    option('Text ' + gray(`(${text})`), () => {
      rl.question('Text: ', (newText) => {
        displayNewFeatChoiceMenu(name, newText, amount, prerequisites, feats, rl);
      });
    }),
    option('Amount ' + gray(`(${amount})`), () => {
      rl.question('Amount: ', (newAmount) => {
        ...
        displayNewFeatChoiceMenu(name, text, amountInt, prerequisites, feats, rl);
      });
    }),
    ...
    option('Add all feats', () => {
      ...
      displayNewFeatChoiceMenu(name, text, amount, prerequisites, getFeats(), rl);
    }),
    option('Add feat', () => {
      rl.question('Feat name: ', (featName) => {
        ...
        if (!feat) {
          ...
          displayNewFeatChoiceMenu(name, text, amount, prerequisites,[...feats, feat], rl);
        }
      });
    }),
    option('Remove feat', () => {
      rl.question('Feat name: ', (featName) => {
        ...
        if (!feat) {
          ...
          displayNewFeatChoiceMenu(name, text, amount, prerequisites, feats.filter((s) => s.id !== feat.id), rl);
        }
      });
    }),
    option(green('Save'), () => {
...
function createFeatChoice(name: string, text: string, prerequisites: Prerequisite[], feats: Feat[]) {
...
function updateEffects(choice: Choice) {
  ...
  choice.options.forEach((opt) => {
    const optionFeatNotPrerequisite = opt.prerequisites
      ...
      .find((prerequisite) => FeatPrerequisite.prototype.isPrototypeOf(prerequisite.prerequisite));
    if (!optionFeatNotPrerequisite) {
      console.log(red(`Could not find feat prerequisite for option ${opt.text}`));
      return;
    }
    const optionFeatPrerequisite = optionFeatNotPrerequisite.prerequisite as FeatPrerequisite;
    const feat = getFeatById(optionFeatPrerequisite.featId);
    if (!feat) {
      console.log(red(`Could not find feat for option ${opt.text}`));
      return;
    }
    const effectFile = path.join(getEffectsDirectory(), `feat_${feat.name.toLowerCase().replace(/[^a-zA-Z0-9._-]/g, '_')}.yml`);
