...
export function displayNewItemProficiencyChoiceMenu(
  name: string,
  text: string,
  amount: number,
...
function createItemProficiencyChoices(
  name: string,
  text: string,
  amount: number,
  prerequisites: Prerequisite[],
...
function createItemProficiencyChoice(
  name: string,
  text: string,
  prerequisites: Prerequisite[],
  items: string[],
  ...
  const choice = new Choice(
    ...
    prerequisites,
    items.map((item) => {
      return new ChoiceOption(uuid.v4(), item, [
        new NotPrerequisite(new ItemProficiencyPrerequisite([item])),
      ]);
    }),
  );
  choice.save();
...
function updateEffects(choice: Choice) {
  ...
  choice.options.forEach((opt) => {
    const optionItemProficiencyNotPrerequisite = opt.prerequisites
      ...
      .find((prerequisite) =>
        ItemProficiencyPrerequisite.prototype.isPrototypeOf(
    ...
    if (!optionItemProficiencyNotPrerequisite) {
      console.log(
        red(
          `Could not find item proficiency prerequisite for option ${opt.text}`,
        ),
      );
      return;
    }
    const optionItemProficiencyPrerequisite =
      optionItemProficiencyNotPrerequisite.prerequisite as ItemProficiencyPrerequisite;
