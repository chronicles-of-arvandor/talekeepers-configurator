...
export function displayNewLanguageChoiceMenu(
  name: string,
  text: string,
  amount: number,
...
function createLanguageChoices(
  name: string,
  text: string,
  amount: number,
  prerequisites: Prerequisite[],
...
function createLanguageChoice(
  ...
  const choice = new Choice(
    ...
    prerequisites,
    languages.map((language) => {
      return new ChoiceOption(uuid.v4(), language.name, [
        new NotPrerequisite(new LanguagePrerequisite([language.id])),
      ]);
    }),
  );
  choice.save();
...
function updateEffects(choice: Choice) {
  ...
  choice.options.forEach((opt) => {
    const optionLanguageNotPrerequisite = opt.prerequisites
      ...
      .find((prerequisite) =>
        LanguagePrerequisite.prototype.isPrototypeOf(prerequisite.prerequisite),
      );
    if (!optionLanguageNotPrerequisite) {
      console.log(
        red(`Could not find language prerequisite for option ${opt.text}`),
      );
      return;
    }
    const optionLanguagePrerequisite =
      optionLanguageNotPrerequisite.prerequisite as LanguagePrerequisite;
