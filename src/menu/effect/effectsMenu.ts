...
export function displayEffectsMenu(rl: readline.Interface) {
  ...
  menu(
    ...
    ...effects.map((effect) =>
      option(path.basename(effect.file), () => {
  ...
  menu(
    ...
    option("Ability effect", () => {
      rl.question("New file name: ", (fileName) => {
        ...
        const effect = new AbilityEffect(effectPath, {}, []);
        displayEffectMenu(effect, rl);
      });
    }),
    option("Character trait effect", () => {
      rl.question("New file name: ", (fileName) => {
        const effectPath = path.join(getEffectsDirectory(), `${fileName}.yml`);
        const effect = new CharacterTraitEffect(effectPath, [], []);
        displayEffectMenu(effect, rl);
      });
    }),
    option("Feat effect", () => {
      rl.question("New file name: ", (fileName) => {
        const effectPath = path.join(getEffectsDirectory(), `${fileName}.yml`);
        const effect = new FeatEffect(effectPath, [], []);
        displayEffectMenu(effect, rl);
      });
    }),
    ...
    option("Initiative bonus effect", () => {
      rl.question("New file name: ", (fileName) => {
        const effectPath = path.join(getEffectsDirectory(), `${fileName}.yml`);
        const effect = new InitiativeBonusEffect(effectPath, 0, []);
        displayEffectMenu(effect, rl);
      });
    }),
    ...
    option("Language effect", () => {
      rl.question("New file name: ", (fileName) => {
        const effectPath = path.join(getEffectsDirectory(), `${fileName}.yml`);
        const effect = new LanguageEffect(effectPath, [], []);
        displayEffectMenu(effect, rl);
      });
    }),
    ...
    option("Skill proficiency effect", () => {
      rl.question("New file name: ", (fileName) => {
        const effectPath = path.join(getEffectsDirectory(), `${fileName}.yml`);
        const effect = new SkillProficiencyEffect(effectPath, [], []);
        displayEffectMenu(effect, rl);
      });
    }),
    option("Speed effect", () => {
      rl.question("New file name: ", (fileName) => {
        const effectPath = path.join(getEffectsDirectory(), `${fileName}.yml`);
        const effect = new SpeedEffect(effectPath, 0, []);
        displayEffectMenu(effect, rl);
      });
    }),
    option("Spell effect", () => {
      rl.question("New file name: ", (fileName) => {
        const effectPath = path.join(getEffectsDirectory(), `${fileName}.yml`);
        const effect = new SpellEffect(effectPath, [], []);
        displayEffectMenu(effect, rl);
      });
    }),
