...
export function displayNewSpellChoiceMenu(
  name: string,
  text: string,
  amount: number,
...
function displaySpellListClassMenu(
  name: string,
  text: string,
  amount: number,
  prerequisites: Prerequisite[],
...
function displaySpellListLevelMenu(
  name: string,
  text: string,
  amount: number,
  prerequisites: Prerequisite[],
  ...
  menu(
    ...
    option("3rd (and below)", () => {
      addSpellList(
        ...
        prerequisites,
        spells,
        [...spellList["3"], ...spellList["2"], ...spellList["1"]],
        rl,
      );
    }),
...
function addSpellList(
  ...
  const invalidSpells = spellNames.filter(
    (spellName) => !allSpells.some((spell) => spell.name === spellName),
  );
  if (invalidSpells.length > 0) {
    console.log(
      red(
        `Found ${invalidSpells.length} unmatched spells: ${invalidSpells.join(
          ", ",
        )}`,
      ),
    );
...
function createSpellChoice(
  ...
  const choice = new Choice(
    ...
    prerequisites,
    spells.map((spell) => {
      return new ChoiceOption(uuid.v4(), spell.name, [
        new NotPrerequisite(new SpellPrerequisite([spell.id])),
      ]);
    }),
  );
  choice.save();
...
function updateEffects(choice: Choice) {
  ...
  choice.options.forEach((opt) => {
    const optionSpellNotPrerequisite = opt.prerequisites
      ...
      .find((prerequisite) =>
        SpellPrerequisite.prototype.isPrototypeOf(prerequisite.prerequisite),
      );
    if (!optionSpellNotPrerequisite) {
      console.log(
        red(`Could not find spell prerequisite for option ${opt.text}`),
      );
      return;
    }
    const optionSpellPrerequisite =
      optionSpellNotPrerequisite.prerequisite as SpellPrerequisite;
    ...
    const effectFile = path.join(
      getEffectsDirectory(),
      `spell_${spell.name.toLowerCase().replace(/[^a-zA-Z0-9._-]/g, "_")}.yml`,
    );
