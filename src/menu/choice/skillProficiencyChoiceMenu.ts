  ...
  menu(
    ...
    option('Name ' + gray(`(${name})`), () => {
      rl.question('Name: ', (newName) => {
        displayNewSkillProficiencyChoiceMenu(newName, text, amount, prerequisites, skills, rl);
      });
    }),
    option('Text ' + gray(`(${text})`), () => {
      rl.question('Text: ', (newText) => {
        displayNewSkillProficiencyChoiceMenu(name, newText, amount, prerequisites, skills, rl);
      });
    }),
    option('Amount ' + gray(`(${amount})`), () => {
      rl.question('Amount: ', (newAmount) => {
        ...
        displayNewSkillProficiencyChoiceMenu(name, text, amountInt, prerequisites, skills, rl);
      });
    }),
    ...
    option('Remove skill', () => {
      menu(
        'Skill',
        ...skills.map((skill) => option(skill.name, () => {
          displayNewSkillProficiencyChoiceMenu(name, text, amount, prerequisites, skills.filter((s) => s !== skill), rl);
        })),
...
function createSkillProficiencyChoice(name: string, text: string, prerequisites: Prerequisite[], skills: Skill[]) {
  const choice = new Choice(
    ...
    skills.map((skill) => {
      return new ChoiceOption(
        ...
        [
          new NotPrerequisite(
            new SkillProficiencyPrerequisite([skill])
          )
        ]
      );
    })
...
function updateEffects(choice: Choice) {
  ...
  choice.options.forEach((opt) => {
    const optionSkillProficiencyNotPrerequisite = opt.prerequisites
      ...
      .find((prerequisite) => SkillProficiencyPrerequisite.prototype.isPrototypeOf(prerequisite.prerequisite));
    if (!optionSkillProficiencyNotPrerequisite) {
      console.log(red(`Could not find skill proficiency prerequisite for option ${opt.text}`));
      return;
    }
    const optionSkillProficiencyPrerequisite = optionSkillProficiencyNotPrerequisite.prerequisite as SkillProficiencyPrerequisite;
    const skill = optionSkillProficiencyPrerequisite.skills[0];
    if (!skill) {
      console.log(red(`Could not find skill for option ${opt.text}`));
      return;
    }
    const effectFile = path.join(getEffectsDirectory(), `skill_proficiency_${skill.name.toLowerCase().replace(/[^a-zA-Z0-9._-]/g, '_')}.yml`);
